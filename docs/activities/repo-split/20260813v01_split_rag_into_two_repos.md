...
1. Split on the write/read seam rather than by data domain — `mcp_db_server` learns its schema, tables, and embedding columns from Postgres at runtime, so the interface is SQL, not Python. Splitting policy-vs-briefs instead would mean duplicating one pipeline behind two config sets.
