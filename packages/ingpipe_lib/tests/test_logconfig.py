"""Unit tests for the logconfig package (ingpipe_lib.logconfig)."""
...
import pytest
from ingpipe_lib.logconfig import HANDLER_NAME, get_logger, setup_logging
...
def logconfig_harness() -> Iterator[LogconfigHarness]:
    ...
    workspace-wide pytest run, other packages' test modules call
    setup_logging at import, leaving a HANDLER_NAME handler on the root
    that would make every setup() here a no-op — so it is detached for
...
def test_both_record_timestamps_are_utc(
    ...
    run_started = datetime.strptime(record["run_timestamp"], "%Y.%m.%d_%H.%M.%SZ").replace(
        tzinfo=UTC
    )
...
def test_appended_runs_carry_distinct_run_timestamps(
    ...
    mock_datetime = mocker.patch("ingpipe_lib.logconfig.datetime")
    mock_now = MagicMock()
    mock_datetime.now.return_value = mock_now

    mock_now.strftime.return_value = "2026.08.13_12.00.00"
    logconfig_harness.setup(log_dir=tmp_path, log_name="run", overwrite=False)
    get_logger("test_module").info("first run")
    logconfig_harness.end_run()

    mock_now.strftime.return_value = "2026.08.13_12.00.01"
    logconfig_harness.setup(log_dir=tmp_path, log_name="run", overwrite=False)
    get_logger("test_module").info("second run")
    logconfig_harness.end_run()
    ...
    assert timestamps == ["2026.08.13_12.00.00", "2026.08.13_12.00.01"]
...
def test_log_name_falls_back_to_log_when_no_caller_frame(
    ...
    mocker.patch("ingpipe_lib.logconfig.inspect.currentframe", return_value=None)
    logconfig_harness.setup(log_dir=tmp_path)
