"""Logging configuration for the workspace's entry-point scripts.
...
`get_logger()` for every module. Kept in ``ingpipe-lib`` so CI never
depends on untracked `.claude/` content; this copy intentionally forks
from the skill copy (skill updates do not propagate by design).
"""
...
def setup_logging(
    ...
    if log_name is None:
        ...
        caller_filepath = frame.f_back.f_globals.get("__file__") if frame and frame.f_back else None
    ...
    # UTC, marked with Z, so records written on different machines order
    # against one another and against a run's manifest (manifest.py
    # already records datetime.now(UTC)).
    ...
    formatter = JsonFormatter(
        "%(run_timestamp)s %(asctime)s %(name)s %(funcName)s %(levelname)s %(message)s"
    )
